...
export class AdminUsuarios implements OnInit {
  ...
  cargarRoles(): void {
    this.usuarioService.getRoles().subscribe({
      next: (response) => this.rolesDisponibles = response,
  ...
  cargarClientes(): void {
    this.clienteService.getClientesList().subscribe({
      next: (response) => {
        ...
        this.clientesDisponibles = clientes.map((cliente: any) => ({ id: cliente.id, nombre: cliente.name, code: cliente.code, status: cliente.status }));
      },
